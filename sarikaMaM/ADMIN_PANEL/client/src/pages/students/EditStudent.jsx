...
export const EditStudent = () => {
    ...
    const fetchStudent = async () => {
      try {
        ...
        if (student) {
          ...
          setErrors({ api: 'Student record not found.' });
        }
      } catch (err) {
        setErrors({ api: 'Failed to load student data.' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ api: error.message || 'Failed to update student profile.' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <h2 className="text-lg font-bold text-slate-800 dark:text-white">Modify Student Profile</h2>
      </div>
