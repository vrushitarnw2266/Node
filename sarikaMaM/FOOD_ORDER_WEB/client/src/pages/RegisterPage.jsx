...
export function RegisterPage({ onRegister, api }) {
  ...
  return (
    <div className="animate-fade-in flex min-h-[60vh] items-center justify-center">
      <div className="glass-card premium-shadow w-full max-w-md rounded-[2.5rem] p-10">
        <h2 className="text-3xl font-bold text-kk-dark mb-2 text-center">Create Account</h2>
        <p className="text-gray-500 text-center mb-8 text-sm">Join the gourmet community today</p>
        
        {error && <div className="mb-6 rounded-2xl bg-red-50 p-4 text-sm text-red-600 border border-red-100">{error}</div>}

        <form className="space-y-5" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-2">
            <label className="font-bold text-xs uppercase tracking-widest text-gray-400">Full Name</label>
            <input 
          ...
          <button type="submit" className="btn-gradient w-full py-4 text-lg mt-4">Register</button>
        </form>

        <p className="mt-8 text-center text-sm text-gray-500">
          Already have an account?{" "}
          <Link to="/login" className="font-bold text-kk-red hover:underline">Login instead</Link>
        </p>
      </div>
    </div>
  );
